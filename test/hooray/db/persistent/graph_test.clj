...
(deftest redis-iterator-test
  (let [data [[1 1 2] [1 2 3] [1 2 4] [1 2 5] [1 3 4]]
        ...
        it (per-g/->redis-iterator {:triple [(pack/hash->bb 1) (pack/hash->bb 2)'?v] :triple-order [:e :a :v]} fix/*key-store*)]
    #_(proto/set-ks fix/*key-store* :eav #_fix/*keyspace* packed)
    ;; (is (= 'data (proto/seek fix/*key-store* :eav (pack/pack-hash-array 1 2))))
    (is (false? (itr/at-end? it)))
    (is (= 3 (-> it itr/key pack/bb->hash)))
    (is (= 4 (-> it itr/next itr/key pack/bb->hash)))
    (is (= 5 (-> it itr/next itr/next itr/key pack/bb->hash)))
...
(deftest redis-iterator-seek-test
  (let [data (concat [[1 1 2]]
    ...
    ;; (is (= 'data (proto/seek fix/*key-store* :eav (pack/pack-hash-array 1 2))))
    (is (false? (itr/at-end? it)))
    (is (= 0 (-> it itr/key pack/bb->hash)))
    (is (= 1 (-> it itr/next itr/key pack/bb->hash)))
    (is (= 2 (-> it itr/next itr/next itr/key pack/bb->hash)))
    (is (= 1000 (-> it (itr/seek (pack/hash->bb 1000)) itr/key pack/bb->hash)))
    (is (= 1001 (-> it (itr/seek (pack/hash->bb 1000)) itr/next itr/key pack/bb->hash)))
    (is (= 9999 (-> it (itr/seek (pack/hash->bb 9999)) itr/key pack/bb->hash)))
    (is (nil? (-> it (itr/seek (pack/hash->bb 9999)) itr/next itr/key)))
    (is (false? (-> it (itr/seek (pack/hash->bb 9999)) itr/at-end?)))
    (is (true? (-> it (itr/seek (pack/hash->bb 9999)) itr/next itr/at-end?)))
    (is (nil? (-> it (itr/seek (pack/hash->bb 10000)) itr/key)))
    (is (true? (-> it (itr/seek (pack/hash->bb 10000)) itr/at-end?)))
    (redis/clear-set fix/redis-conn :eav)))
